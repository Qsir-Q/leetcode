...
时间复杂度：O(nlogn)  排序稳定性：不稳定，如果数组本来就是排序的，会退化为 O(n^2)

递归方式；把大问题分化为小问题，最小是两个节点，马上可以得出结果；选定一个基点；把比基点小的放到左边，比基点大的放到右边；然后把 基点放到中间的位置，因为只承诺左右两边比base大或者小，所以换了不会导致乱序
